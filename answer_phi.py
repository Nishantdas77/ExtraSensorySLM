"""
QA engine (Phi-3.5-mini) 
Usage:
  python3 answer_phi.py --timeline timeline.json --question "..."         
  python3 answer_phi.py --timeline timeline.json --question "..." --no-llm 
  python3 answer_phi.py --timeline timeline.json --questions q.txt --out answers.txt
"""
import argparse, json, re, sys

# ===================== MODEL WRAPPER — Phi-3.5-mini ==========================
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, pipeline

MODEL_NAME = "microsoft/Phi-3.5-mini-instruct"
_pipe = None
_use_cache = True          # flipped to False if the cache API mismatches
...
def _load():
    ...
    # NOTE: trust_remote_code is deliberately OFF.
    # Phi-3.5's bundled modeling code calls cache.seen_tokens, which newer
    # transformers removed -> 'DynamicCache' object has no attribute 'seen_tokens'.
    # Recent transformers has native Phi3 support, so we use that instead.
    # eager attention avoids the missing flash_attn package.
...
def ask_llm(prompt: str, max_new_tokens: int = 220) -> str:
    ...
    args = {
        ...
        "pad_token_id": _pipe.tokenizer.pad_token_id,
    }
    try:
        out = _pipe(messages, **args)
    except AttributeError as e:
        if "seen_tokens" in str(e) and _use_cache:
            # last-resort fallback: run without the KV cache (slower, always works)
            print("[cache API mismatch -> retrying with use_cache=False]", file=sys.stderr)
            _use_cache = False
...
# plain everyday synonyms that count as naming an activity DIRECTLY
DIRECT_WORDS = {
    "lying_down": ["lie down", "lying", "lay down", "laying", "reclin"],
    "sitting": ["sit", "sitting", "seated"],
    "standing_in_place": ["stand", "standing"],
    "standing_and_moving": ["standing and moving", "moving around"],
    "walking": ["walk", "walking", "walked", "stroll"],
    "running": ["run", "running", "ran", "jog", "sprint"],
    "bicycling": ["bicycl", "cycling", "cycle", "bike", "biking"],   # NOT "pedal"/"wheeled"
}
...
def solve(tl, q, intent=None):
    ...
    if intent:                                   # LLM-parsed intent, then validated
        ...
        ql = q.lower()
        # An open-world CONCEPT word makes this a Task-4 question no matter how the
        # sentence starts ("Was the user using a wheeled..." is open, not verify).
        has_concept_word = any(k in ql for k in CONCEPTS) or \
                           re.search(r"prolonged|sustained|consistent with", ql)
        if re.search(r"\bhow long\b|how much time|total time|\bduration\b", ql):
            ...
            t = "compare"
        elif has_concept_word:
            t = "open"
        elif re.search(r"^\s*(is|was|did|does|has|were|are)\b", ql) and kw:
            t = "verify"          # a yes/no question about a named activity
        elif re.search(r"what (activity|is the user|was the user).*(doing|perform)|"
                       r"which activity|identify the activity", ql):
            t = "identify"        # the brief's canonical Task-1 question

        # A question that names NO activity and has no open-world concept word
        # cannot be verify/duration/count/onset/compare/open/identify about one
        # activity -- the model invented it. Such questions are overviews.
        has_concept = any(k in ql for k in CONCEPTS) or \
                      re.search(r"prolonged|sustained|consistent with", ql)
        if not kw and not has_concept and t not in ("summary", "unsupported", "identify"):
            t, acts = "summary", []
        elif t in NEEDS_ACTIVITY and not kw:
            t, acts = "summary", []
    ...
    if t == "open":
        ...
        kw_acts = find_activities(q)          # activities NAMED IN THE QUESTION
        # If the question names an activity DIRECTLY (its own name or a plain
        # synonym), that wins. If the activity only appears via a concept word
        # ("wheeled", "pedal-based"), keep the descriptive open-world label.
        direct = [a for a in kw_acts
                  if a.replace("_", " ") in q.lower() or a in q.lower()
                  or any(w in q.lower() for w in DIRECT_WORDS.get(a, []))]
        if direct:
            target, label = direct, None
        elif c_acts:
            target = c_acts
        else:
            target = kw_acts or (acts if acts else None)
